...
async function authenticate({ email, password, ipAddress }) {
    ...
    // localStorage.setItem(accountsKey, JSON.stringify(basicDetails(account)));
    // save refresh token
...
async function getRefreshToken(token) {
    ...
    console.log("from acc.serv"+refreshToken)
    if (!refreshToken ) throw 'Invalid tokennn';
    return refreshToken;
}
...
async function sendVerificationEmail(account, origin) {
    let message;
    if (origin) {
        const verifyUrl = `${origin}/Parfume/#/account/verify-email?token=${account.verificationToken}`;
